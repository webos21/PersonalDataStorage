...
import RegularRecordForm from './RegularRecordForm';
import { FIELD_CONFIG as RegularRecordFieldConfig } from './RegularRecordField';
import RegularRecordColumns from './RegularRecordColumns';
...
const RegularRecord = () => {
    ...
    const labelByKey = useMemo(
        () => Object.fromEntries(RegularRecordFieldConfig.map((field: any) => [field.name, field.label])),
        []
    );

    const tableKeys = useMemo(() => Object.keys(rows?.[0] || {}).slice(0, 6), [rows]);
    const formKeys = useMemo(
        () => Object.keys(rows?.[0] || {}).filter((key) => key !== 'id' && key !== 'rrId'),
        [rows, labelByKey]
    );
    ...
    const columns = useMemo(() => RegularRecordColumns(tableKeys, labelByKey, openForm), [tableKeys, labelByKey]);
    ...
    return (
        <PageLayout>
            <PageHeader icon={RefreshCcw} title="정기결제기록" desc="정기납입 기록 관리" iconClass="bg-blue-100 text-blue-600" />
            <div className="fms-table-wrap flex-1 flex flex-col relative">
                <TableToolbar
                    ...
                    searchPlaceholder="정기기록 검색 (Ctrl+K)"
                ...
                <DataTable
                    ...
                    data={rows}
                    isLoading={isLoading}
                    emptyMessage="등록된 정기기록 데이터가 없습니다."
                    manualSorting
                />
