...
const PasswordBookColumns = (
    ...
    const dataColumns = (tableKeys.length ? tableKeys : ['id']).map((key) =>
        c.accessor(key, {
            ...
            enableSorting: false,
            cell: (info) => (
                <span className="text-sm text-zinc-800">{formatDateCellValue(key, info.getValue?.(), labelByKey[key] || key)}</span>
            )
        })
    );
...
export default PasswordBookColumns;
