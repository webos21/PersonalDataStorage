...
import PbFormAdd from './PbFormAdd';
import PbFormEdit from './PbFormEdit';
...
class PasswordBook extends Component {
  constructor(props) {
    ...
    this.state = {
      emptyId: -1,
      dataSet: [],
      currentData: { id: -1, siteUrl: '', siteName: '', siteType: '', myId: '', myPw: '', memo: '' },
      totalCount: 0,
      itemsPerPage: 10,
      totalPage: 0,
      currentPage: 0,
  ...
  genEmptyObj() {
    ...
    return { id: newId, siteUrl: '', siteName: '', siteType: '', myId: '', myPw: '', memo: '' };
  }

  renderTableList(dataArray) {
    return dataArray.length === 0 ? <tr><td colSpan="3" className="p-4 text-center">No Data</td></tr> :
      dataArray.map(data => (
        <tr key={'pb-' + data.id} onClick={(e) => this.handleEdit(data, e)} className="hover:bg-gray-50 cursor-pointer border-b">
          <td className="p-2 border">{data.siteName}<br /><a href={data.siteUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">{data.siteUrl}</a></td>
          <td className="p-2 border">{data.siteType}</td>
          <td className="p-2 border">{data.myId}</td>
        </tr>
      ));
  }
    ...
    return (
      <div className="space-y-6">
        <div className="bg-white p-4 shadow rounded">
          <div className="font-bold text-lg mb-2">Search <small className="text-gray-500">PasswordBook</small></div>
        ...
        <div className="bg-white p-4 shadow rounded">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-lg">Password List <small className="text-gray-500">(Total: {this.state.totalCount})</small></h2>
            <button onClick={this.handleAdd} className="px-4 py-2 bg-green-600 text-white rounded">+ 추가</button>
          </div>
          <table className="w-full border-collapse">
            <thead className="bg-gray-100">
              <tr><th className="p-2 border">이름</th><th className="p-2 border">유형</th><th className="p-2 border">ID</th></tr>
            </thead>
        ...
        <PbFormAdd key={"PbFormAdd-" + this.state.currentData.id} modalFlag={this.state.modalFlagAdd} modalToggle={this.modalToggleAdd} dataFromParent={this.state.currentData} callbackFromParent={this.dataChangedCallback} />
        <PbFormEdit key={"PbFormEdit-" + this.state.currentData.id} modalFlag={this.state.modalFlagEdit} modalToggle={this.modalToggleEdit} dataFromParent={this.state.currentData} callbackFromParent={this.dataChangedCallback} />
      </div>
    );
  }
}
