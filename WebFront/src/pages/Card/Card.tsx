...
import { FIELD_CONFIG as CardFieldConfig } from './CardField';
...
const Card = () => {
    ...
    const labelByKey = useMemo(
        () => ({
            ...Object.fromEntries(CardFieldConfig.map((field: any) => [field.name, field.label])),
            validPeriod: '유효월년(MM/YY)'
        }),
        []
    );

    const tableKeys = useMemo(
        () => ['company', 'cardName', 'cardNumber', 'validPeriod', 'chargeDate', 'notUsed', 'memo'],
        []
    );
    const formKeys = useMemo(
        () => Object.keys(rows?.[0] || {}).filter((key) => key !== 'id' && key !== 'cardId'),
        [rows, labelByKey]
    );
    ...
    const columns = useMemo(() => CardColumns(tableKeys, labelByKey, openForm), [tableKeys, labelByKey]);
    ...
    return (
        <PageLayout>
            <PageHeader icon={CreditCard} title="카드" desc="카드 정보 관리" iconClass="bg-blue-100 text-blue-600" />
