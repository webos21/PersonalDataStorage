...
const AccountCodeAdd = (props) => {
    ...
    const onSubmit = (data, e) => {
        ...
        fetch(REQ_URI, { method: 'POST', headers: Utils.auth.makeAuthHeader(), body: formData })
            ...
            .catch((error) => {
                console.log(error);
                setError('acodeId', { message: error.message });
            });
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
            <form onSubmit={handleSubmit(onSubmit)} className="bg-gray-100 p-0 rounded-xl shadow-2xl w-[400px] overflow-hidden">
                <div className="flex justify-between items-center p-4 bg-white border-b">
                    <h2 className="text-lg font-bold">계정코드 추가</h2>
                    <button type="button" onClick={() => props.modalToggle(false)} className="text-gray-500 hover:text-black">
                        ✕
                    </button>
                </div>
                <div className="p-6">
                    <div className="mb-4">
                        ...
                        <input
                            ...
                            className={`w-full p-2 border rounded ${errors.acodeId ? 'border-red-500' : 'border-gray-300'}`}
                        />
                ...
                <div className="flex justify-end gap-2 p-4 bg-white border-t">
                    ...
                    <button type="button" onClick={() => props.modalToggle(false)} className="px-4 py-2 bg-gray-500 text-white rounded">
                        취소
                    </button>
                </div>
            </form>
