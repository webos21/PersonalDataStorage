...
import RealEstateForm, { FIELD_CONFIG as RealEstateFieldConfig } from './RealEstateForm';
import RealEstateColumns from './RealEstateColumns';
import api from './api';
...
const RealEstate = () => {
    ...
    const labelByKey = useMemo(
        () => Object.fromEntries(RealEstateFieldConfig.map((field: any) => [field.name, field.label])),
        []
    );

    const tableKeys = useMemo(() => Object.keys(rows?.[0] || {}).slice(0, 6), [rows]);
    const formKeys = useMemo(
        () => Object.keys(rows?.[0] || {}).filter((key) => key !== 'id' && key !== 'reId'),
        [rows, labelByKey]
    );
    ...
    const columns = useMemo(() => RealEstateColumns(tableKeys, labelByKey, openForm), [tableKeys, labelByKey]);
    ...
    return (
        <PageLayout>
            <PageHeader icon={MapPinHouse} title="부동산" desc="부동산 정보 관리" iconClass="bg-blue-100 text-blue-600" />
            <div className="fms-table-wrap flex-1 flex flex-col relative">
                <TableToolbar
                    ...
                    searchPlaceholder="부동산 검색 (Ctrl+K)"
                ...
                <DataTable
                    ...
                    data={rows}
                    isLoading={isLoading}
                    emptyMessage="등록된 부동산 데이터가 없습니다."
                    manualSorting
                />
