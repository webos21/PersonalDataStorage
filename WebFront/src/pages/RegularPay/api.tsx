...
const ENDPOINT = '/regularPay';
const QUERY_KEY = 'regular-pay';
const service = { async list(page: number, perPage: number, q: string | undefined) { const response = await ApiClient.get(ENDPOINT, { params: { page, perPage, q } }); return response.data ?? {}; }, async create(payload: FormData) { const response = await ApiClient.post(ENDPOINT, payload); return response.data?.data?.[0] ?? null; }, async update(payload: FormData) { const response = await ApiClient.put(ENDPOINT, payload); return response.data?.data?.[0] ?? null; }, async delete(id: string | number) { const response = await ApiClient.delete(ENDPOINT, { params: { rpId: id } }); return response.data?.deletedRows ?? 0; } };
