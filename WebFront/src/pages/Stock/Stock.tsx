...
import StockForm, { FIELD_CONFIG as StockFieldConfig } from './StockForm';
...
const Stock = () => {
    ...
    const labelByKey = useMemo(
        () => Object.fromEntries(StockFieldConfig.map((field: any) => [field.name, field.label])),
        []
    );

    const tableKeys = useMemo(() => Object.keys(rows?.[0] || {}).slice(0, 6), [rows]);
    const formKeys = useMemo(
        () => Object.keys(rows?.[0] || {}).filter((key) => key !== 'id' && key !== 'stockId'),
        [rows, labelByKey]
    );
    ...
    const columns = useMemo(() => StockColumns(tableKeys, labelByKey, openForm), [tableKeys, labelByKey]);
    ...
    return (
        <PageLayout>
            <PageHeader icon={ChartLine} title="주식" desc="주식 정보 관리" iconClass="bg-blue-100 text-blue-600" />
            <div className="fms-table-wrap flex-1 flex flex-col relative">
                <TableToolbar
                    ...
                    searchPlaceholder="주식 검색 (Ctrl+K)"
