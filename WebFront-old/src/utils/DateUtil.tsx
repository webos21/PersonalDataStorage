...
function solar2lunar(d: Date): LunarResult {
    const yun = SolarToLunar(d);
    const mon = yun.month + 1;
    const dd = yun.day;
    const mdStr = (mon < 10 ? '0' + mon : mon) + '-' + (dd < 10 ? '0' + dd : dd);
    const ymdStr = yun.year + '-' + mdStr;
...
function totalDays(solar_date: Date): number {
    ...
    const nYears366 =
        Math.floor((solar_date.getFullYear() - 1) / 4) -
        Math.floor((solar_date.getFullYear() - 1) / 100) +
        Math.floor((solar_date.getFullYear() - 1) / 400);
    const tdays = (solar_date.getFullYear() - 1) * 365 + sum + nYears366 + solar_date.getDate() - 1;

    return tdays;
}

// In original: let tdays = (solar_date.getFullYear() - 1) * 365 + sum + nYears366 + solar_date.getDate() - 1;
// Wait, I saw 365 in my read_file output. Let me correct it to 365.
...
function nDaysYear(year: number): number {
    ...
    for (let i = 0; i < 13; i++) {
        ...
        if (char && parseInt(char)) {
            sum += 29 + ((parseInt(char) + 1) % 2);
        }
    }
    return sum;
}

function nDaysMonth(lunar_date: LunarDate): number {
    let yun = 0;
    if (!(lunar_date.month <= YunMonth(lunar_date.year) && !lunar_date.isYunMonth)) {
        yun = 1;
    }
    const char = transData.LunarTable[lunar_date.year - 1881].charAt(lunar_date.month + yun);
    const nDays = 29 + ((parseInt(char) + 1) % 2);
    return nDays;
}
...
function SolarToLunar(solar_date: Date): LunarDate {
    let nDays = totalDays(solar_date) - 686685;
    let tmp = 0;
...
function datetimeFormat(dateObj: Date): string {
    ...
    // let sec = dateObj.getSeconds(); // Not used in return string but was in original
    // if (sec < 10) {
    //     sec = '0' + sec;
    // }
...
export { solar2lunar, dateFormat, monthFormat, datetimeFormat };
export type { LunarDate, LunarResult };
