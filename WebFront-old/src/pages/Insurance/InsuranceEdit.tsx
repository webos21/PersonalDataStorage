    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg shadow-xl w-[500px] overflow-hidden">
                ...
                <div className="p-4 border-b font-bold text-lg">보험 수정</div>
                <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    {fields.map(f => (
                        <div key={f.name} className="flex flex-col">
                            <label className="text-sm font-medium">{f.label}</label>
                            <Controller name={f.name} control={control} defaultValue={f.name === 'contractDate' ? Utils.date.dateFormat(new Date(props.dataFromParent.contractDate)) : f.name === 'maturityDate' ? Utils.date.dateFormat(new Date(props.dataFromParent.maturityDate)) : props.dataFromParent[f.name]} render={({field}) => f.type === 'textarea' ? 
                    ...
                    <div className="flex gap-4">
                        <label>계약상태:</label>
                        <Controller name="contractStatus" control={control} defaultValue={"" + props.dataFromParent.contractStatus} render={({field}) => (
