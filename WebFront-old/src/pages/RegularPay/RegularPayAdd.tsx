...
const RegularPayAdd = props => {
    ...
    const fields = [
        { name: 'title', label: '정기납명', type: 'text', placeholder: '정기납명을 입력해 주세요.' },
        ...
        { name: 'monthDay', label: '정기납일', type: 'number', placeholder: '정기납일을 입력해 주세요.' },
        { name: 'sdate', label: '납입시작', type: 'date', placeholder: '' },
        { name: 'edate', label: '납입종료', type: 'date', placeholder: '' },
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg shadow-xl w-[500px] overflow-hidden">
                <div className="p-4 border-b font-bold text-lg">정기납입 추가</div>
                <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    {fields.map(f => (
                        <div key={f.name} className="flex flex-col">
                            <label className="text-sm font-medium">{f.label}</label>
                            <Controller name={f.name} control={control} defaultValue={f.name === 'wdate' ? Utils.date.dateFormat(new Date(props.dataFromParent.wdate)) : f.name === 'sdate' ? Utils.date.dateFormat(new Date(props.dataFromParent.sdate)) : f.name === 'edate' ? Utils.date.dateFormat(new Date(props.dataFromParent.edate)) : props.dataFromParent[f.name]} render={({field}) => f.type === 'textarea' ? 
