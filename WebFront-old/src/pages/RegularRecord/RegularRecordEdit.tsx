...
const RegularRecordEdit = props => {
    ...
    const fields = [
        { name: 'wdate', label: '등록일', type: 'date', placeholder: '' },
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg shadow-xl w-[500px] overflow-hidden">
                <input type="hidden" {...register('rrId')} defaultValue={props.dataFromParent.id} />
                <div className="p-4 border-b font-bold text-lg">정기납입기록 수정</div>
                <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div className="flex flex-col">
                        <label className="text-sm font-medium">납입선택</label>
                        <Controller name="regularPayId" control={control} defaultValue={props.dataFromParent.regularPayId} render={({field}) => (
                            <select {...field} className={`p-2 border rounded ${errors.regularPayId ? 'border-red-500' : 'border-gray-300'}`}>
                                <option value={-1}>정기납입을 선택해 주세요.</option>
                                {rpayList.map(data => <option key={data.id} value={data.id}>{data.title}</option>)}
                            </select>
                        )} />
                        {errors.regularPayId && <p className="text-red-500 text-xs">{errors.regularPayId.message}</p>}
                    </div>
                    {fields.map(f => (
                        <div key={f.name} className="flex flex-col">
                            <label className="text-sm font-medium">{f.label}</label>
                            <Controller name={f.name} control={control} defaultValue={f.name === 'wdate' ? Utils.date.dateFormat(new Date(props.dataFromParent.wdate)) : props.dataFromParent[f.name]} render={({field}) => f.type === 'textarea' ? 
