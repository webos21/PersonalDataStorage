...
import AcodeSelector from '../../components/AcodeSelector/AcodeSelector';
...
const AccountCode = () => {
    ...
    const handleAdd = () => {
        ...
        setPageData(p => ({ ...p, currentData: { id: newId, accountCode: '', title: '' }, modalFlagAdd: !p.modalFlagAdd, emptyId: newId }));
    };
    ...
    const renderTableList = (dataArray) => {
        ...
        if (!filteredData.length) return <tr><td colSpan="3" className="p-4 text-center">No Data</td></tr>;
    ...
    return (
        <ComponentSkeleton>
            <div className="flex justify-between items-center mb-4 gap-2">
                <SearchBar width="50%" keyword={pageData.keyword} onSearchGo={(e) => { e.preventDefault(); setPageData(p => ({...p, keyword: e.target.searchKeyword.value })); }} onReset={() => setPageData(p => ({...p, keyword: ''}))} />
                <button onClick={handleAdd} className="px-4 py-2 bg-green-600 text-white rounded">추가</button>
            </div>
            <table className="w-full border-collapse">
                <thead className="bg-gray-200">
                    <tr><th className="p-2 text-left">ID</th><th className="p-2 text-left">계정코드</th><th className="p-2 text-left">분류명</th></tr>
                </thead>
                <tbody>{renderTableList(acodes)}</tbody>
            </table>
            {pageData.modalFlagAdd && <AccountCodeAdd modalFlag={pageData.modalFlagAdd} modalToggle={modalToggleAdd} dataFromParent={pageData.currentData} callbackFromParent={dataChangedCallback} />}
            {pageData.modalFlagEdit && <AccountCodeEdit modalFlag={pageData.modalFlagEdit} modalToggle={modalToggleEdit} dataFromParent={pageData.currentData} callbackFromParent={dataChangedCallback} />}
