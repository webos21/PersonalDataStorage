...
const stats = [
    { title: '전체 운행 차량', value: '0 (0%)', hint: '전체 0대', color: 'bg-emerald-500', icon: '▶' },
    { title: '전체 운행 거리', value: '0 km', hint: '-', color: 'bg-blue-500', icon: '◎' },
    { title: '전체 운행 시간', value: '-', hint: '-', color: 'bg-violet-500', icon: '◔' },
    { title: '전체 트립 건수', value: '0', hint: '-', color: 'bg-amber-500', icon: '#' }
];
                    ...
                    {stats.map((item) => (
                        <div key={item.title} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                            <div className="flex items-start gap-3">
                                <div className={`mt-1 flex h-8 w-8 items-center justify-center rounded-lg text-white ${item.color}`}>{item.icon}</div>
                                <div>
