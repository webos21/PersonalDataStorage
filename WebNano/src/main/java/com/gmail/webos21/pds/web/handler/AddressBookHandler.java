...
public class AddressBookHandler implements UriHandler {

	private AddressBookRepo abRepo;

	public AddressBookHandler() {
		PdsDbManager pdb = PdsDbManager.getInstance();
		abRepo = pdb.getRepository(AddressBookRepo.class);
	}
	...
	private RouteResult processDelete(Map<String, String> headers, IHTTPSession session, String uri) {
		...
		String abId = params.get("abId");
		int deletedRows = abRepo.deleteRow(Long.parseLong(abId));
	...
	private RouteResult processPut(Map<String, String> headers, NanoHTTPD.IHTTPSession session, String uri,
		...
		AddressBook aRow = new AddressBook(Long.parseLong(abId), fullName, mobile, category, telephone, fax, email,
				homepage, postcode, address, memo);
		abRepo.updateRow(aRow);
	...
	private RouteResult processPost(Map<String, String> headers, NanoHTTPD.IHTTPSession session, String uri,
		...
		AddressBook aRow = new AddressBook(null, fullName, mobile, category, telephone, fax, email, homepage, postcode,
				address, memo);
		abRepo.updateRow(aRow);
	...
	private RouteResult processGet(Map<String, String> headers, IHTTPSession session, String uri,
		...
		List<AddressBook> rows = null;
		if (session.getParameters().get("q") != null) {
			String q = session.getParameters().get("q").get(0);
			if (q != null && q.length() > 0) {
				rows = abRepo.findRows(session.getParameters().get("q").get(0));
			}
		}
		if (rows == null) {
			rows = abRepo.findRows();
		}
