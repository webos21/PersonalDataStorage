...
public class RecordHandler implements UriHandler {

	private RecordRepo recordRepo;

	public RecordHandler() {
		PdsDbManager pdb = PdsDbManager.getInstance();
		recordRepo = pdb.getRepository(RecordRepo.class);
	}
	...
	private RouteResult processDelete(Map<String, String> headers, IHTTPSession session, String uri) {
		...
		int deletedRows = recordRepo.deleteRow(Long.parseLong(recordId));
	...
	private RouteResult processPut(Map<String, String> headers, NanoHTTPD.IHTTPSession session, String uri,
		...
		Record aRow = new Record(Long.parseLong(recordId), wd, title, Long.parseLong(deposit),
				Long.parseLong(withdrawal), accountCode, memo);
		recordRepo.updateRow(aRow);
	...
	private RouteResult processPost(Map<String, String> headers, NanoHTTPD.IHTTPSession session, String uri,
		...
		Record aRow = new Record(null, wd, title, Long.parseLong(deposit), Long.parseLong(withdrawal), accountCode,
				memo);
		recordRepo.updateRow(aRow);
	...
	private RouteResult processGet(Map<String, String> headers, IHTTPSession session, String uri,
		...
		if (session.getParameters().get("q") != null) {
			...
			if (q != null && q.length() > 0) {
				rows = recordRepo.findRows(session.getParameters().get("q").get(0));
			}
		}
		if (rows == null) {
			rows = recordRepo.findRows();
		}
	...
	private RouteResult processGetCalendar(Map<String, String> headers, IHTTPSession session, String year,
		...
		List<Record> rows = recordRepo.findRows(Integer.parseInt(year), Integer.parseInt(month));
