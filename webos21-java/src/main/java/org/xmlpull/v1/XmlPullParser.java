 ...
 *
 *         xpp.<a href=
"#setInput">setInput</a>( new StringReader ( "&lt;foo>Hello World!&lt;/foo>" ) );
...
public interface XmlPullParser {
	...
	/**
	 * Signalize that parser is at the very beginning of the document and nothing
	 * was read yet. This event type can only be observed by calling getEvent()
	 ...
	 * <p>
	 * <strong>NOTE:</strong> subsequent calls to <a href="#next()">next()</a> or
	 * <a href="#nextToken()">nextToken()</a> may result in exception being thrown.
	 *
	 * @see #next
	 * @see #nextToken
	 */
	 ...
	 * <a href="#nextToken()">nextToken()</a> when a start tag was read. The name of
	 * start tag is available from getName(), its namespace and prefix are available
	 * from getNamespace() and getPrefix() if
	 ...
	 * <a href="#nextToken()">nextToken()</a> when an end tag was read. The name of
	 * start tag is available from getName(), its namespace and prefix are available
	 * from getNamespace() and getPrefix().
	 *
	 * @see #next
	 * @see #nextToken
	 * @see #getName
	 ...
	 * <p>
	 * <strong>Please note:</strong> <a href="#next()">next()</a> will accumulate
	 * multiple events into one TEXT event, skipping IGNORABLE_WHITESPACE,
	 * PROCESSING_INSTRUCTION and COMMENT events, In contrast,
	 * <a href="#nextToken()">nextToken()</a> will stop reading text when any other
	 * event is observed. Also, when the state was reached by calling next(), the
	 * text value will be normalized, whereas getText() will return unnormalized
	 * content in the case of nextToken(). This allows an exact roundtrip without
	 * changing line ends when examining low level events, whereas for high level
	 * applications the text is normalized appropriately.
	 *
	 * @see #next
	 * @see #nextToken
	 * @see #getText
	 ...
	 * <a href="#nextToken()">nextToken()</a>. A call to next() will accumulate
	 * various text events into a single event of type TEXT. The text contained in
	 * the CDATA section is available by calling getText().
	 *
	 * @see #nextToken
	 * @see #getText
	 */
	 ...
	 * <a href="#nextToken()">nextToken()</a> only. The entity name is available by
	 * calling getName(). If available, the replacement text can be obtained by
	 * calling getText(); otherwise, the user is responsible for resolving the
	 * entity reference. This event type is never returned from next(); next() will
	 * accumulate the replacement text and other text events to a single TEXT event.
	 *
	 * @see #nextToken
	 * @see #getText
	 */
	 ...
	 * locations. The ignorable whitespace string is available by calling getText()
	 *
	 * <p>
	 ...
	 *
	 * Ignorable whitespace is skipped by next() automatically; this event type is
	 * never returned from next().
	 *
	 * @see #nextToken
	 * @see #getText
	 */
	...
	/**
	 * An XML processing instruction declaration was just read. This event type is
	 * available only via <a href="#nextToken()">nextToken()</a>. getText() will
	 * return text that is inside the processing instruction. Calls to next() will
	 * skip processing instructions automatically.
	 * 
	 * @see #nextToken
	 * @see #getText
	 */
	 ...
	 * <a href="#nextToken()">nextToken()</a> only; calls to next() will skip
	 * comments automatically. The content of the comment can be accessed using the
	 * getText() method.
	 *
	 * @see #nextToken
	 * @see #getText
	 ...
	 * <a href="#nextToken()">nextToken()</a> only. The unparsed text inside the
	 * doctype is available via the getText() method.
	 *
	 * @see #nextToken
	 * @see #getText
	 */
	...
	/**
	 * This array can be used to convert the event type integer constants such as
	 * START_TAG or TEXT to to a string. For example, the value of TYPES[START_TAG]
	 * is the string "START_TAG".
	 *
	 * This array is intended for diagnostic output only. Relying on the contents of
	 * the array may be dangerous since malicious applications may alter the array,
	...
	/**
	 * This feature determines whether the parser processes namespaces. As for all
	 * features, the default value is false.
	 * <p>
	 * <strong>NOTE:</strong> The value can not be changed during parsing an must be
	 * set before parsing.
	 *
	 * @see #getFeature
	 * @see #setFeature
	 ...
	 * attribute access methods. Like all features, the default value is false. This
	 * feature cannot be changed during parsing.
	 *
	 * @see #getFeature
	 * @see #setFeature
	 */
	...
	/**
	 * This feature determines whether the document declaration is processed. If set
	 * to false, the DOCDECL event type is reported by nextToken() and ignored by
	 * next().
	 *
	 * If this feature is activated, then the document declaration must be processed
	 * by the parser.
	 *
	 * <p>
	 * <strong>Please note:</strong> If the document type declaration was ignored,
	 * entity references may cause exceptions later in the parsing process. The
	 * default value of this feature is false. It cannot be changed during parsing.
	 *
	 * @see #getFeature
	 * @see #setFeature
	 */
	...
	/**
	 * If this feature is activated, all validation errors as defined in the XML 1.0
	 * specification are reported. This implies that FEATURE_PROCESS_DOCDECL is true
	 * and both, the internal and external document type declaration will be
	 * processed.
	 * <p>
	 * <strong>Please Note:</strong> This feature can not be changed during parsing.
	 ...
	 * called before the first call to next or nextToken. Otherwise, an exception is
	 * thrown.
	 * <p>
	 * Example: call setFeature(FEATURE_PROCESS_NAMESPACES, true) in order to switch
	 * on namespace processing. The initial settings correspond to the properties
	 * requested from the XML Pull Parser factory. If none were requested, all
	 ...
	 * <p>
	 * <strong>Please note:</strong> unknown features are <strong>always</strong>
	 * returned as false.
	 *
	 ...
	 * The event type is set to the initial value START_DOCUMENT. Setting the reader
	 * to null will just stop parsing and reset parser state, allowing the parser to
	 * free internal resources such as parsing buffers.
	 */
	void setInput(Reader in) throws XmlPullParserException;

	/**
	 * Sets the input stream the parser is going to process. This call resets the
	 * parser state and sets the event type to the initial value START_DOCUMENT.
	 *
	 * <p>
	 * <strong>NOTE:</strong> If an input encoding string is passed, it MUST be
	 * used. Otherwise, if inputEncoding is null, the parser SHOULD try to determine
	 * input encoding following XML 1.0 specification (see below). If encoding
	...
	/**
	 * Returns the input encoding if known, null otherwise. If setInput(InputStream,
	 * inputEncoding) was called with an inputEncoding value other than null, this
	 * value must be returned from this method. Otherwise, if inputEncoding is null
	 * and the parser supports the encoding detection feature
	 * (http://xmlpull.org/v1/doc/features.html#detect-encoding), it must return the
	 * detected encoding. If setInput(Reader) was called, null is returned. After
	 * first call to next if XML declaration was present this method will return
	 * encoding declared.
	 */
	 ...
	 * Construction of Internal Entity Replacement Text</a>. If
	 * FEATURE_PROCESS_DOCDECL or FEATURE_VALIDATION are set, calling this function
	 * will result in an exception -- when processing of DOCDECL is enabled, there
	 * is no need to the entity replacement text manually.
	 *
	 * <p>
	 * The motivation for this function is to allow very small implementations of
	 * XMLPULL that will work in J2ME environments. Though these implementations may
	 * not be able to process the document type declaration, they still can work
	 * with known DTDs by using this function.
	 *
	 * <p>
	 * <b>Please notes:</b> The given value is used literally as replacement text
	 ...
	 * contain any other entity reference (if it contains any entity reference there
	 * will be no further replacement).
	 *
	 * <p>
	 * <b>Note:</b> The list of pre-defined entity names will always contain
	 * standard XML entities such as amp (&amp;amp;), lt (&amp;lt;), gt (&amp;gt;),
	 * quot (&amp;quot;), and apos (&amp;apos;). Those cannot be redefined by this
	 * method!
	 *
	 * @see #setInput
	 * @see #FEATURE_PROCESS_DOCDECL
	 ...
	 * Default namespace declaration (xmlns='...') will have null as prefix. If the
	 * given index is out of range, an exception is thrown.
	 * <p>
	 * <b>Please note:</b> when the parser is on an END_TAG, namespace prefixes that
	 * were declared in the corresponding START_TAG are still accessible although
	 * they are no longer in scope.
	 */
	String getNamespacePrefix(int pos) throws XmlPullParserException;

	/**
	 * Returns the namespace URI for the given position in the namespace stack If
	 * the position is out of range, an exception is thrown.
	 * <p>
	 * <b>NOTE:</b> when parser is on END_TAG then namespace prefixes that were
	 * declared in corresponding START_TAG are still accessible even though they are
	 * not in scope
	 */
	String getNamespaceUri(int pos) throws XmlPullParserException;

	/**
	 * Returns the URI corresponding to the given prefix, depending on current state
	 * of the parser.
	 *
	 * <p>
	 * If the prefix was not declared in the current scope, null is returned. The
	 * default namespace is included in the namespace table and is available via
	 * getNamespace (null).
	 *
	 * <p>
	...
	/**
	 * Returns the current depth of the element. Outside the root element, the depth
	 * is 0. The depth is incremented by 1 when a start tag is reached. The depth is
	 ...
	 * This method is especially useful to provide meaningful error messages and for
	 * debugging purposes.
	 */
	String getPositionDescription();

	/**
	 * Returns the current line number, starting from 1. When the parser does not
	 * know the current line number or can not determine it, -1 is returned (e.g.
	 * for WBXML).
	 *
	...
	/**
	 * Returns the current column number, starting from 0. When the parser does not
	 * know the current column number or can not determine it, -1 is returned (e.g.
	 * for WBXML).
	 *
	 ...
	 * For IGNORABLE_WHITESPACE, this is always true. For TEXT and CDSECT, false is
	 * returned when the current event text contains at least one non-white space
	 * character. For any other event type an exception is thrown.
	 *
	 * <p>
	 * <b>Please note:</b> non-validating parsers are not able to distinguish
	 * whitespace and ignorable whitespace, except from whitespace outside the root
	 * element. Ignorable whitespace is reported as separate event, which is exposed
	 * via nextToken only.
	 *
	 */
	 ...
	 *
	 * See description of nextToken() for detailed description of possible returned
	 * values for different types of events.
	 *
	 * <p>
	 * <strong>NOTE:</strong> in case of ENTITY_REF, this method returns the entity
	 * replacement text (or null if not available). This is the only case where
	...
	/**
	 * Returns the buffer that contains the text of the current event, as well as
	 * the start offset and length relevant for the current event. See getText(),
	 * next() and nextToken() for description of possible returned values.
	 *
	 * <p>
	 * <strong>Please note:</strong> this buffer must not be modified and its
	 * content MAY change after a call to next() or nextToken(). This method will
	 * always return the same value as getText(), except for ENTITY_REF. In the case
	 * of ENTITY ref, getText() returns the replacement text and this method returns
	 * the actual input buffer containing the entity name. If getText() returns
	 * null, this method returns null as well and the values returned in the holder
	 ...
	 * @see #nextToken
	 *
	 * @param holderForStartAndLength Must hold an 2-element int array into which
	 *                                the start offset and length values will be
	 *                                written.
	 * @return char buffer that contains the text of the current event (null if the
	 *         current event has no text associated).
	 */
	...
	/**
	 * Returns the namespace URI of the current element. The default namespace is
	 * represented as empty string. If namespaces are not enabled, an empty String
	 * ("") is always returned. The current event must be START_TAG or END_TAG;
	 ...
	 * returned when namespaces are enabled. When namespace processing is disabled,
	 * the raw name is returned. For ENTITY_REF events, the entity name is returned.
	 * If the current event is not START_TAG, END_TAG, or ENTITY_REF, null is
	 * returned.
	 * <p>
	...
	/**
	 * Returns the prefix of the current element. If the element is in the default
	 * namespace (has no prefix), null is returned. If namespaces are not enabled,
	 * or the current event is not START_TAG or END_TAG, null is returned.
	 */
	String getPrefix();
	 ...
	 * attribute has no namespace. Throws an IndexOutOfBoundsException if the index
	 * is out of range or the current event type is not START_TAG.
	 *
	 * <p>
	...
	/**
	 * Returns the local name of the specified attribute if namespaces are enabled
	 * or just attribute name if namespaces are disabled. Throws an
	 * IndexOutOfBoundsException if the index is out of range or current event type
	 * is not START_TAG.
	 *
	 * @param index zero-based index of attribute
	...
	/**
	 * Returns the prefix of the specified attribute Returns null if the element has
	 * no prefix. If namespaces are disabled it will always return null. Throws an
	 * IndexOutOfBoundsException if the index is out of range or current event type
	 * is not START_TAG.
	 *
	 * @param index zero-based index of attribute
	...
	/**
	 * Returns the type of the specified attribute If parser is non-validating it
	 * MUST return CDATA.
	 *
	 * @param index zero-based index of attribute
	...
	/**
	 * Returns if the specified attribute was not in input was declared in XML. If
	 * parser is non-validating it MUST always return false. This information is
	 * part of XML infoset:
	 *
	 * @param index zero-based index of attribute
	...
	/**
	 * Returns the given attributes value. Throws an IndexOutOfBoundsException if
	 * the index is out of range or current event type is not START_TAG.
	 *
	 * <p>
	 * <strong>NOTE:</strong> attribute value must be normalized (including entity
	 * replacement text if PROCESS_DOCDECL is false) as described in
	...
	/**
	 * Get next parsing event - element content will be coalesced and only one TEXT
	 * event must be returned for whole element content (comments and processing
	 * instructions will be ignored and entity references must be expanded or
	 * exception must be thrown if entity reference can not be expanded). If element
	 * content is empty (content is "") then no TEXT event will be reported.
	 *
	 * <p>
	 ...
	 * (identified by URI: http://xmlpull.org/v1/doc/features.html#xml-roundtrip) is
	 * enabled it is possible to do XML document round trip ie. reproduce exectly on
	 * output the XML input using getText(): returned content is always unnormalized
	 * (exactly as in input). Otherwise returned content is end-of-line normalized
	 * as described <a href="http://www.w3.org/TR/REC-xml#sec-line-ends">XML 1.0
	 * End-of-Line Handling</a> and. Also when this feature is enabled exact content
	 * of START_TAG, END_TAG, DOCDECL and PROCESSING_INSTRUCTION is available.
	 *
	 * <p>
	 ...
	 * <dd>null
	 * <dt>START_TAG
	 * <dd>null unless FEATURE_XML_ROUNDTRIP enabled and then returns XML tag, ex:
	 * &lt;tag attr='val'>
	 * <dt>END_TAG
	 * <dd>null unless FEATURE_XML_ROUNDTRIP id enabled and then returns XML tag,
	 * ex: &lt;/tag>
	 * <dt>TEXT
	 ...
	 * <dd>return characters that are determined to be ignorable white space. If the
	 * FEATURE_XML_ROUNDTRIP is enabled all whitespace content outside root element
	 * will always reported as IGNORABLE_WHITESPACE otherwise reporting is optional.
	 * <br>
	 ...
	 * from &lt;?pi foo?> otherwise it may be exact PI content or concatenation of
	 * PI target, space and data so for example for &lt;?target data?> string
	 * &quot;target data&quot; may be returned if FEATURE_XML_ROUNDTRIP is false.
	 * <dt>COMMENT
	 * <dd>return comment content ex. 'foo bar' from &lt;!--foo bar-->
	 * <dt>ENTITY_REF
	 * <dd>getText() MUST return entity replacement text if PROCESS_DOCDECL is false
	 * otherwise getText() MAY return null, additionally getTextCharacters() MUST
	 * return entity name (for example 'entity_name' for &amp;entity_name;). <br>
	 * <b>NOTE:</b> this is the only place where value returned from getText() and
	 * getTextCharacters() <b>are different</b> <br>
	 ...
	 * &amp;amp; &amp;lt; &amp;gt; &amp;quot; &amp;apos; are reported as well and
	 * are <b>not</b> reported as TEXT tokens but as ENTITY_REF tokens! This
	 * requirement is added to allow to do roundtrip of XML documents!
	 * <dt>DOCDECL
	 * <dd>if FEATURE_XML_ROUNDTRIP is true or PROCESS_DOCDECL is false then return
	 * what is inside of DOCDECL for example it returns:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * otherwise if FEATURE_XML_ROUNDTRIP is false and PROCESS_DOCDECL is true then
	 * what is returned is undefined (it may be even null)</dd>
	 * </dl>
	 *
	 * <p>
	 * <strong>NOTE:</strong> there is no guarantee that there will only one TEXT or
	 * IGNORABLE_WHITESPACE event from nextToken() as parser may chose to deliver
	 * element content in multiple tokens (dividing element content into chunks)
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * <strong>NOTE:</strong> XMLDecl (&lt;?xml ...?&gt;) is not reported but its
	 * content is available through optional properties (see class description
	 * above).
	 *
	 * @see #next
	 * @see #START_TAG
	 ...
	 * do match. null will match any namespace and any name. If the test is not
	 * passed, an exception is thrown. The exception text indicates the parser
	 * position, the expected event and the current event that is not meeting the
	 * requirement.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * The motivation for this function is to allow to parse consistently both empty
	 * elements and elements that has non empty content, for example for input:
	 * <ol>
	 * <li>&lt;tag&gt;foo&lt;/tag&gt;
	 * <li>&lt;tag&gt;&lt;/tag&gt; (which is equivalent to &lt;tag/&gt; both input
	 * can be parsed with the same code:
	 * 
	 * <pre>
	 *   p.nextTag()
	 ...
	 *
	 * <p>
	 * <strong>Warning:</strong> Prior to API level 14, the pull parser returned by
	 * {@code
	...
	/**
	 * Call next() and return event if it is START_TAG or END_TAG otherwise throw an
	 * exception. It will skip whitespace TEXT before actual tag if any.
	 *
	 * <p>
	 * essentially it does this
	 * 
