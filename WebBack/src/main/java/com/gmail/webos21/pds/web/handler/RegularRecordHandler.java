...
public class RegularRecordHandler implements UriHandler {

	private RegularRecordRepo rrRepo;

	public RegularRecordHandler() {
		PdsDbManager pdb = PdsDbManager.getInstance();
		rrRepo = pdb.getRepository(RegularRecordRepo.class);
	}
	...
	private RouteResult processDelete(Map<String, String> headers, IHTTPSession session, String uri) {
		...
		String rrId = params.get("rrId");
		int deletedRows = rrRepo.deleteRow(Long.parseLong(rrId));
	...
	private RouteResult processPut(Map<String, String> headers, NanoHTTPD.IHTTPSession session, String uri,
		...
		String rrId = params.get("rrId");
		String regularPayId = params.get("regularPayId");
		...
		RegularRecord aRow = new RegularRecord(Long.parseLong(rrId), Long.parseLong(regularPayId), wd, title,
				Long.parseLong(deposit), Long.parseLong(withdrawal), accountCode, memo);
		rrRepo.updateRow(aRow);
	...
	private RouteResult processPost(Map<String, String> headers, NanoHTTPD.IHTTPSession session, String uri,
		...
		RegularRecord aRow = new RegularRecord(null, Long.parseLong(regularPayId), wd, title, Long.parseLong(deposit),
				Long.parseLong(withdrawal), accountCode, memo);
		rrRepo.updateRow(aRow);
	...
	private RouteResult processGet(Map<String, String> headers, IHTTPSession session, String uri,
		...
		List<RegularRecord> rows = null;
		if (session.getParameters().get("q") != null) {
			String q = session.getParameters().get("q").get(0);
			if (q != null && q.length() > 0) {
				rows = rrRepo.findRows(session.getParameters().get("q").get(0));
			}
		}
		if (rows == null) {
			rows = rrRepo.findRows();
		}
