...
public class AccountClassHandler implements UriHandler {

	private AccountClassRepo accountClassRepo;

	public AccountClassHandler() {
		PdsDbManager pdb = PdsDbManager.getInstance();
		accountClassRepo = pdb.getRepository(AccountClassRepo.class);
	}
	...
	private RouteResult processDelete(Map<String, String> headers, IHTTPSession session, String uri) {
		...
		int deletedRows = accountClassRepo.deleteRow(Long.parseLong(acId));
	...
	private RouteResult processPut(Map<String, String> headers, NanoHTTPD.IHTTPSession session, String uri,
		...
		AccountClass aRow = new AccountClass(Long.parseLong(acId), title);
		accountClassRepo.updateRow(aRow);
	...
	private RouteResult processGet(Map<String, String> headers, IHTTPSession session, String uri,
		...
		List<AccountClass> rows = null;
		if (session.getParameters().get("q") != null) {
			String q = session.getParameters().get("q").get(0);
			if (q != null && q.length() > 0) {
				rows = accountClassRepo.findRows(session.getParameters().get("q").get(0));
			}
		}
		if (rows == null) {
			rows = accountClassRepo.findRows();
		}
